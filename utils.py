...
def farthest_insertion(points):
    ...
    while remaining_points:
        ...
        farthest_point = max(remaining_points, key=lambda point: min(haversine_distance(point, path_point) for path_point in current_path))

        # Trouver l'indice du point le plus proche dans le chemin actuel
        nearest_index = min(range(len(current_path)), key=lambda i: haversine_distance(farthest_point, current_path[i]))
...
# Liste de points GPS
points_gps = [(latitude1, longitude1), (latitude2, longitude2), (latitude3, longitude3), (latitude4, longitude4)]
